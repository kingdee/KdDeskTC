        ...
        if let Ok(msg_in) = RendezvousMessage::parse_from_bytes(&bytes) {
            if let Some(rendezvous_message::Union::request_relay(rf)) = msg_in.union {
                if !key.is_empty() && rf.licence_key != key {
                    // added by fantacy.
                    log::info!(
                        "invalid key, rf.licence_key:{}, key:{}",
                        rf.licence_key, key
                    );
                    return;
                }
                if !rf.uuid.is_empty() {
                    let mut peer = PEERS.lock().await.remove(&rf.uuid.clone());
                    if let Some(peer) = peer.as_mut() {
                        log::info!("UUID:{} IP:{} Type:CreateRelay1 target:{}", rf.uuid.clone(), addr.to_string(), rf.id.clone());
                        ...
                        {
                            log::info!("UUID:{} IP:{} Type:CloseRelay target:{} err {}", rf.uuid.clone(), addr.to_string(), rf.id.clone(), err.to_string());
                        } else {
                            log::info!("UUID:{} IP:{} Type:CloseRelay target:{} ", rf.uuid.clone(), addr.to_string(), rf.id.clone());
                        }
                        USAGE.write().await.remove(&id);
                    } else {
                        log::info!("UUID:{} IP:{} Type:CreateRelay2 target:{}", rf.uuid.clone(), addr.to_string(), rf.id.clone());
