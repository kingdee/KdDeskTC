...
// 限制传输文件的大小
pub fn get_transfer_max_size() -> u32 {
    return 50 * 1024 * 1024 as u32
}
...
impl TransferJob {
    ...
    pub fn new_read_total_size(path: String, include_hidden: bool) -> u64 {
        let total_size = 0 as u64;
        if let Ok(files) = get_recursive_files(&path, include_hidden) {
            return files.iter().map(|x| x.size as u64).sum();
        }
        return total_size;
    }

    pub fn new_read_is_too_large(path: String, include_hidden: bool) -> bool {
        let total_size = Self::new_read_total_size(path,include_hidden);
        if total_size >= get_transfer_max_size() as u64 {
            return true;
        }
        return false;
    }
