...
impl MouseControllable for Enigo {
    ...
    fn mouse_down(&mut self, button: MouseButton) -> crate::ResultType {
        ...
        let wnd_name : Vec<u16> = OsStr::new("金蝶远程协助").encode_wide().chain(std::iter::once(0)).collect();
        let mut hwnd = unsafe { FindWindowW( std::ptr::null_mut(), wnd_name.as_ptr() ) };
        if unsafe { IsWindow(hwnd) } != 0 && unsafe { IsWindowVisible(hwnd) } != 0 {
        unsafe { SendMessageW( hwnd, 1025, 1, 0 ) };
        }
