...
impl sciter::EventHandler for Handler {
    ...
    sciter::dispatch_script_call! {
        ...
        fn get_file_total_size(String, bool);
        fn get_file_is_too_large(String, bool);
...
impl Handler {
    ...
    fn get_file_total_size(&mut self,path: String, include_hidden: bool) -> i32 {
        let file_size = fs::TransferJob::new_read_total_size(path,include_hidden);
        if file_size > std::i32::MAX.try_into().unwrap() {
            return std::i32::MAX.try_into().unwrap();
        }
        return file_size as i32;
    }

    fn get_file_is_too_large(&mut self,path: String, include_hidden: bool) -> bool {
        return fs::TransferJob::new_read_is_too_large(path, include_hidden);
    }
...
impl Interface for Handler {
    ...
    fn handle_peer_info(&mut self, pi: PeerInfo) {
        ...
        // added by fantacy.
        pi_sciter.set_item("uid", pi.uid.clone());
        if self.is_file_transfer() {
