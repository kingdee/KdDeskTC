...
use crate::ui::cm::mpsc::UnboundedSender;
...
impl ConnectionManager {
    ...
    fn check_click_time(&mut self, id: i32) {
        log::info!("cm -> check_click_time {}", id);
        if let Some(client) = CLIENTS.read().unwrap().get(&id) {
            log::info!("cm -> found client check_click_time {}", id);
            allow_err!(client.tx.send(Data::ClickTime(0)));
            log::info!("cm -> found client, send Data::ClickTime {}", id);
        };
    }

    fn get_click_time(&self) -> f64 {
        log::info!("cm -> get_click_time ");
        CLICK_TIME.load(Ordering::SeqCst) as f64
    }
    ...
    fn add_connection(
        ...
        log::info!("==addConnection== id {} is_file_transfer {} keyboard {} file {} ", 
            id, is_file_transfer, keyboard, file);
                                            ...
                                            Ok(Some(data)) => {
                                                match data {
                                                    Data::Login{id, is_file_transfer, port_forward, peer_id, name, authorized, keyboard, clipboard, audio, file, file_transfer_enabled} => {
                                                        log::debug!("conn_id: {} peer_id {} ", id, peer_id.clone());
                                                        conn_id = id;
                                                        let cur_permission = Config::get_peer_permission(peer_id.clone(),"file".to_owned());
                                                        tx_file.send(ClipboardFileData::Enable((id, file_transfer_enabled))).ok();
                                                        cm.add_connection(id, is_file_transfer, port_forward, peer_id, name, authorized, keyboard, clipboard, audio, file && cur_permission != "0" , tx.clone());
                                                    }
                                                    Data::Close => {
                                                    ...
                                                    Data::ClickTime(ms) => {
                                                        log::info!("ui -> cm interface : Data::ClickTime ");
