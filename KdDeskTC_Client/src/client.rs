...
impl Client {
    ...
    async fn _start(peer: &str, conn_type: ConnType) -> ResultType<(Stream, bool)> {
        ...
        let from_pk = Config::get_key_pair().1;
        let from_uuid = if let Ok(id) = machine_uid::get() {
            log::info!("machine uid: {}", id);
            id
        } else {
            String::from_utf8(from_pk)?
        };
        ...
        for i in 1..=3 {
            ...
            msg_out.set_punch_hole_request(PunchHoleRequest {
                ...
                licence_key: hbb_common::config::get_public_key().to_owned(),
    ...
    async fn secure_connection(peer_id: &str, signed_id_pk: Vec<u8>, conn: &mut Stream) -> ResultType<()> {
        ...
        //let rs_pk = get_rs_pk("MsU4IVn2luPwhB2eq72tzsEgnylWps2Gp3LyixJ07nQ=");
        let rs_pk = get_rs_pk(hbb_common::config::get_public_key().as_str());
        let mut sign_pk = None;
        /*if signed_id_pk.is_empty() {
            log::error!("secure_connection, signed_id_pk empty.");
        }*/
        if !signed_id_pk.is_empty() && rs_pk.is_some() {
            ...
            if sign_pk.is_none() {
                ...
                //log::error!("Handshake success");
            }
        }
        //log::error!("secure_connection, 1.");
