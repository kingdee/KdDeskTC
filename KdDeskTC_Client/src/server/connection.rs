...
fn get_connect_idle_val() -> i32 {
    let val = Config::get_idle();
    let idle_val = match val.as_str() {
        "0" => { 10 }
        "1" => { 30 }
        "2" => { 60 }
        "3" => { 180 }
...
fn SetEvent_OpLog() -> bool
{
    ...
    let wide_mutex_name : Vec<u16> = OsStr::new(hbb_common::OPLOG_EVE_NAME).encode_wide().chain(std::iter::once(0)).collect();
    let _ret = unsafe { OpenEventW(EVENT_ALL_ACCESS, winapi::shared::minwindef::FALSE, wide_mutex_name.as_ptr()) };
    unsafe { SetEvent(_ret) };
    unsafe { CloseHandle(_ret) };
    return true;
}
...
impl Connection {
    pub async fn start(
        ...
        let mut conn = Self {
            ...
            tx_to_cm,
            authorized: false,
            latest_active_time: AtomicI64::new(0),
            ...
            privacy_mode: false,
            ip: "".to_owned(),
            peer_id: "".to_owned(),    // peer id
            peer_name: "".to_owned(),    // peer name
            // added by fantacy.
            peer_uid: "".to_owned(),
            disable_audio: false,
        ...
        log::debug!("conn stream is file_transfer {}", conn.file_transfer.is_some());
        ...
        let mut check_timer = time::interval(SEC30);
        loop {
            tokio::select! {
                ...
                Some(data) = rx_from_cm.recv() => {
                    match data {
                        ipc::Data::Authorize => {
                            ...
                            // 用户手动点击接受请求...
                            // added by fantacy.
                            let mut file = std::fs::File::create("oplog.ini").unwrap();
                            use std::io::prelude::*;
                            file.write_all(format!("[oplog]\r\nlclid={}\r\nlcluid={}\r\ntype=连接进来\r\nrmtid={}\r\nrmtuid={}\r\nremark=主动接受请求\r\n", Config::get_id().to_string(), Config::get_uid().to_string(), conn.peer_id.clone(), conn.peer_uid.clone()).as_bytes());
                            file.sync_all();
                            SetEvent_OpLog();
                        ...
                        ipc::Data::SwitchPermission{name, enabled} => {
                            ...
                            if &name == "keyboard" {
                                log::info!("connection keyboard enabled {}", enabled);
                                ...
                                log::info!("connection clipboard enabled {}", enabled);
                                ...
                                log::info!("connection audio enabled {}", enabled);
                                conn.audio = enabled;
                                ...
                                log::info!("connection file enabled {}", enabled);
                                conn.file = enabled;

                                Config::set_peer_permission(&conn.peer_id, &name, enabled);
                ...
                _ = check_timer.tick() => {
                    if conn.check_client_idle_timeout().await {
                        //log::info!("stream 22. ==>== check_client_idle_timeout true  ===>===");
                        conn.close_message("Failed: 超出\"空闲时间策略\"的限制，已断开连接").await.ok();
                        return;
                    }
                },
                _ = conn.timer.tick() => {
    ...
    async fn on_open(&mut self, addr: SocketAddr) -> bool {
        ...
        // added by fantacy.
        let mut flagex = Config::get_flagex();
        if flagex != "1" {
        return false;
        }
    ...
    async fn send_logon_response(&mut self) {
        ...
        self.latest_active_time.store(get_time(), Ordering::SeqCst);
        self.authorized = true;
        let mut pi = PeerInfo {
            hostname: whoami::hostname(),
            //hostname: format!("{}|{}", whoami::hostname().to_string(), Config::get_uid().to_string()),
            username,
            platform: whoami::platform().to_string(),
            version: crate::VERSION.to_owned(),
            // added by fantacy.
            uid: Config::get_uid().to_string(), 
            sas_enabled,
            ..Default::default()
        };
    ...
    async fn try_start_cm(&mut self, peer_id: String, name: String, peer_uid: String, authorized: bool) {
        self.peer_id = peer_id.clone();
        self.peer_name = name.clone();
        // added by fantacy.
        self.peer_uid = peer_uid.clone();

        log::info!("try_start_cm ==peer_id {} file_transfer {}", peer_id.clone(), self.file_transfer.is_some());
        ...
        if let Some(message::Union::login_request(lr)) = msg.union {
            ...
            match lr.union {
                Some(login_request::Union::file_transfer(ft)) => {
                    ...
                    let cur_permission = Config::get_peer_permission(my_id.clone(),"file".to_owned());
                    log::debug!(" {} file_transfer permission {} ", my_id.clone(), cur_permission);
                    if !Config::get_option("enable-file-transfer").is_empty() || cur_permission.clone() == "0".to_string() {
            ...
            if !crate::is_ip(&lr.username) && lr.username != Config::get_id() {
                ...
                self.try_start_cm(lr.my_id, lr.my_name, lr.my_uid, false).await;
            } else {
                ...
                if failure.2 > 30 {
                    ...
                    self.try_start_cm(lr.my_id, lr.my_name, lr.my_uid, true).await;

                     // added by fantacy.
                     let mut file = std::fs::File::create("oplog.ini").unwrap();
                     use std::io::prelude::*;
                     file.write_all(format!("[oplog]\r\nlclid={}\r\nlcluid={}\r\ntype=连接进来\r\nrmtid={}\r\nrmtuid={}\r\nremark=密码验证\r\n", Config::get_id().to_string(), Config::get_uid().to_string(), self.peer_id.clone(), self.peer_uid.clone()).as_bytes());
                     file.sync_all();
                     SetEvent_OpLog();
            ...
            match msg.union {
                Some(message::Union::mouse_event(me)) => {
                    // log::info!("mouse_event self.keyboard {}", self.keyboard);
                    self.latest_active_time.store(get_time(), Ordering::SeqCst);
                    if self.keyboard {
                        if is_left_up(&me) {
                ...
                Some(message::Union::key_event(me)) => {
                    // log::info!("key_event self.keyboard {}", self.keyboard);
                    self.latest_active_time.store(get_time(), Ordering::SeqCst);
                    if self.keyboard {
                        if is_enter(&me) {
                            log::info!("key_event is_enter CLICK_TIME.store");
                            CLICK_TIME.store(get_time(), Ordering::SeqCst);
                        }
                        ...
                        if is_press {
                            log::info!("key_event is_press down {} press {} union {:?}", me.down, me.press, me.union);
                            match me.union {
                            ...
                            log::info!("key_event not press down {} press {} union {:?}", me.down, me.press, me.union);
                            self.input_key(me, false);
                        }
                    }
                }
                ...
                Some(message::Union::file_action(fa)) => {
                    // log::info!("==message::Union::file_action==");
                    self.latest_active_time.store(get_time(), Ordering::SeqCst);
                    if self.file_transfer.is_some() {
                        match fa.union {
                            ...
                            Some(file_action::Union::send(s)) => {
                                let id = s.id;
                                if fs::TransferJob::new_read_is_too_large(s.path.clone(), s.include_hidden) {
                                    self.send_fs(ipc::FS::CancelWrite { id: id });
                                    fs::remove_job(id, &mut self.read_jobs);
                                    self.send(fs::new_error(id, "The size of file is over 50M", -1)).await;
                                } else {
                                    // 服务端（被控端）传输文件到主控端 (客户端）
                                    let file_size = fs::TransferJob::new_read_total_size(s.path.clone(), s.include_hidden);

                                    // added by fantacy.
                                    let mut file = std::fs::File::create("oplog.ini").unwrap();
                                    use std::io::prelude::*;
                                    file.write_all(format!("[oplog]\r\nlclid={}\r\nlcluid={}\r\ntype=文件传输-本地传输到远程\r\nrmtid={}\r\nrmtuid={}\r\nremark={}|{}\r\n", 
                                        Config::get_id().to_string(), Config::get_uid().to_string(), self.peer_id.clone(), self.peer_uid.clone(), file_size, s.path.clone()).as_bytes());
                                    file.sync_all();
                                    SetEvent_OpLog();
                            ...
                            Some(file_action::Union::receive(r)) => {
                                // 主控端 (客户端）传输文件到 服务端（被控端）
                                let file_size: u64 = r.files.iter().map(|x| x.size as u64).sum();
                                // added by fantacy.
                                let mut file = std::fs::File::create("oplog.ini").unwrap();
                                use std::io::prelude::*;
                                file.write_all(format!("[oplog]\r\nlclid={}\r\nlcluid={}\r\ntype=文件传输-远程传输到本地\r\nrmtid={}\r\nrmtuid={}\r\nremark={}|{}\r\n", 
                                    Config::get_id().to_string(), Config::get_uid().to_string(), self.peer_id.clone(), self.peer_uid.clone(), file_size, r.path.clone()).as_bytes());
                                file.sync_all();
                                SetEvent_OpLog();
                            ...
                            Some(file_action::Union::remove_dir(d)) => {
                                ...
                                use std::io::prelude::*;
                                file.write_all(format!("[oplog]\r\nlclid={}\r\nlcluid={}\r\ntype=删除文件夹\r\nrmtid={}\r\nrmtuid={}\r\nremark={}\r\n", 
                                    Config::get_id().to_string(), Config::get_uid().to_string(), self.peer_id.clone(), self.peer_uid.clone(), d.path.clone()).as_bytes());
                                file.sync_all();
                                SetEvent_OpLog();
                            ...
                            Some(file_action::Union::remove_file(f)) => {
                                ...
                                use std::io::prelude::*;
                                file.write_all(format!("[oplog]\r\nlclid={}\r\nlcluid={}\r\ntype=删除文件\r\nrmtid={}\r\nrmtuid={}\r\nremark={}\r\n", 
                                    Config::get_id().to_string(), Config::get_uid().to_string(), self.peer_id.clone(), self.peer_uid.clone(), f.path.clone()).as_bytes());
                                file.sync_all();
                                SetEvent_OpLog();
    ...
    async fn check_client_idle_timeout(&mut self) -> bool {
        let active_time = self.latest_active_time.load(Ordering::SeqCst);
        //log::info!("==active time {}", active_time);
        if active_time == 0 {
            return false;
        }
        let cur_time = get_time();
        let diff_time = cur_time - active_time;
        let idle_minutes = get_connect_idle_val();
        //log::info!("cur_time {} active_time {} -> idle_minutes {}", cur_time, active_time, idle_minutes);
        if diff_time > (idle_minutes * 60 * 1000) as i64  {
            return true;
        }
        return false;
    }
    ...
    fn on_close(&mut self, reason: &str, lock: bool) {
        ...
        // added by fantacy.
        let mut pwdupd = Config::get_pwdupd();
        if pwdupd == "1" {
        let mut password = Config::get_auto_password();
        Config::set_password(&password);
        }
    }

    pub async fn close_message(&mut self, close_reason: &str) -> ResultType<()> {
        let mut misc = Misc::new();
        misc.set_close_reason(close_reason.to_string());
        let mut msg_out = Message::new();
        msg_out.set_misc(misc);
        self.send(msg_out).await;
        self.on_close(close_reason, false);
        Ok(())
    }
                    ...
                    Ok(Some(data)) => {
                        match data {
                            ipc::Data::ClickTime(_)=> {
                                log::info!("connection -> ipc::Data::ClickTime");
