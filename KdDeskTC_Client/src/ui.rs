...
pub fn start(args: &mut [String]) {
    ...
    frame.set_title(&crate::get_app_name_cn());
    #[cfg(target_os = "macos")]
        ...
        && args.len() > 1
    {
        ...
        #[cfg(windows)]
        {
            ...
            let cur_hwnd = frame.get_hwnd() as HWND;
            let mut stype = args[0].to_owned();
            let mut sid = args[1].to_owned();
            let prop_name : Vec<u16> = OsStr::new(format!("kddesktc{}-{}", stype.to_string(), sid.to_string()).as_str()).encode_wide().chain(std::iter::once(0)).collect();
            unsafe { SetPropW(cur_hwnd, prop_name.as_ptr(), 0xff0101 as HANDLE) };
        }
...
fn start_tray() -> hbb_common::ResultType<()> {
    ...
    app.add_menu_item("打开窗口", |_| {
        crate::run_me(Vec::<&str>::new()).ok();
        //crate::run_me(vec![""]);
        // added by fantacy.
        //let path = inner_main2().expect("Couldn't");
        //let exe = format!("{}\\KdDeskTC.exe", path.display());
        //std::process::Command::new(&exe).arg("").spawn();
        //let cmd = std::env::current_exe()?;
        //std::process::Command::new(cmd).args("").spawn();
    ...
    app.set_timer(std::time::Duration::from_millis(1000), move |app| {
        ...
        if stopped != old.1 {
            ...
            if stopped == 1 {
                old.0 = app.add_menu_item(&crate::lang::translate("Stop service".to_owned()), |_| {
                    ipc::set_option("stop-service", "Y");
                    Ok::<_, systray::Error>(())
                })?;
            } else {
                old.0 = app.add_menu_item(&crate::lang::translate("Start Service".to_owned()), |_| {
...
// added by fantacy.
/*#[cfg(windows)]
fn start_tray_2() -> hbb_common::ResultType<()> {
    let mut app = systray::Application::new()?;
    let icon = include_bytes!("./tray-icon.ico");
    app.set_icon_from_buffer(icon, 32, 32).unwrap();
    // modified by fantacy.
    app.add_menu_item("Open Window", |_| {
    //app.add_menu_item("打开主界面", |_| {
        log::info!("start_tray_2, open.");
        crate::run_me(Vec::<&str>::new()).ok();
        Ok::<_, systray::Error>(())
    })?;
    app.add_menu_item("Exit", |_| {
            log::info!("start_tray_2, exit.");
            Ok::<_, systray::Error>(())
        })?;
    allow_err!(app.wait_for_message());
