...
// added by fantacy.
pub fn check_software_update_ex() {
    let handle = std::thread::spawn(move || allow_err!(_check_software_update()));
    handle.join().unwrap();
}
#[tokio::main(flavor = "current_thread")]
async fn _check_software_update() -> hbb_common::ResultType<()> {
    ...
    println!("_check_software_update,1,{}", crate::VERSION.to_owned());
    ...
    if let Some(Ok((bytes, _))) = socket.next_timeout(30_000).await {
        if let Ok(msg_in) = RendezvousMessage::parse_from_bytes(&bytes) {
            if let Some(rendezvous_message::Union::software_update(su)) = msg_in.union {
                println!("_check_software_update,2,{}", su.url.to_owned());
                let version = hbb_common::get_version_from_url(&su.url);
                if get_version_number(&version) > get_version_number(crate::VERSION) {
                   
                    // added by fantacy.
                    let mut file = std::fs::File::create("result3.ini").unwrap();
                    use std::io::prelude::*;
                    file.write_all(format!("[result]\r\nupdurl={}\r\n", su.url.to_owned()).as_bytes());
                    file.sync_all();

                    *SOFTWARE_UPDATE_URL.lock().unwrap() = su.url;
                }
            } else {
                println!("let Some(rendezvous_message::Union::software_update(su)) = msg_in.union");
            }
        } else {
            println!("let Ok(msg_in) = RendezvousMessage::parse_from_bytes(&bytes)");
        }
    } else {
        println!("let Some(Ok((bytes, _))) = socket.next_timeout(30_000).await");
    }
    Ok(())
}
...
// added by fantacy.
#[inline]
pub fn get_app_name_cn() -> String {
    //format!("{}V{}",hbb_common::config::APP_NAME_CN, crate::version::VERSION)
    format!("{}",hbb_common::config::APP_NAME_CN)
}