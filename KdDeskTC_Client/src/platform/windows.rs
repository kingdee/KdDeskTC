...
fn send_sas() {
    ...
    let mut changeValue = 0;
    let mut changeFlag = false;
    if let Ok(sasVal) = readSASGeneration() {
        if sasVal != 1 {
            if let Ok(_) = writeSASGeneration(1) {
                changeFlag = true;
                changeValue = sasVal;
            }
        }
    ...
    if changeFlag {
        writeSASGeneration(changeValue);
    }
}

fn readSASGeneration() -> ResultType<u32> {
    let regKey = RegKey::predef(HKEY_LOCAL_MACHINE);
    let path = Path::new("Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\System");
    let system = regKey.open_subkey(&path)?;
    let value: u32 = system.get_value("SoftwareSASGeneration")?;
    return Ok(value);
}

fn writeSASGeneration(val: u32) -> ResultType<bool> {
    let regKey = RegKey::predef(HKEY_LOCAL_MACHINE);
    let path = Path::new("Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\System");
    let (settings, disp) = regKey.create_subkey(&path)?;
    settings.set_value("SoftwareSASGeneration",&val)?;
    return Ok(true);
}
...
// added by fantacy.
pub fn start_svc() -> ResultType<()> {
    let cmds = "sc start KdDeskTC";
    //std::thread::sleep(std::time::Duration::from_millis(6000));
    run_cmds(cmds.to_string(), false)?;
    Ok(())
}
pub fn stop_svc() -> ResultType<()> {
    let cmds = "sc stop KdDeskTC";
    run_cmds(cmds.to_string(), false)?;
    Ok(())
}
...
pub fn uninstall_me() -> ResultType<()> {
    // added by fantacy.
    //let mut cfgpath = hbb_common::config::Config::path("KdDeskTC").with_extension("toml");
    let mut cur_cfgpath = hbb_common::config::Config::path("");
    ...
    //println!("uninstall_me, sys_cfgpath:{}.", sys_cfgpath.display());
...
netsh advfirewall firewall delete rule name=\"{app_name} Service\"
    ",
        ...
        subkey = subkey,
        start_menu = start_menu,
        cur_cfgpath = cur_cfgpath.display(),
        sys_cfgpath = sys_cfgpath,
        ext = ext,
    );
    run_cmds(cmds, true)
}
