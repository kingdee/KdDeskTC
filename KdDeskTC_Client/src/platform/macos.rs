...
pub fn start_svc() -> ResultType<()> {
    println!("===start_svc===");
    let exe = std::env::current_exe().unwrap_or_default();
    let active_user_id = get_active_userid();
    println!("exe {:?} active_user_id {} ", exe.to_str(), active_user_id.as_str());
    // open -a /Users/dev/git/rustdesk/KdDeskTC --args --server
